...
let convient piece x y = 
  ...
  let vr=voisin x y 1 in 
  let vb=voisin x y 2 in 
  let vl=voisin x y 3 in 

  let rec convient_aux piece n= 
    if(n==5)
    then 
      false
    else 
    ...
    then 
      ...
      convient_aux (turnR piece 1) (n+1) 
  in 
  convient_aux v 1
;;
...
let lesPieces=pdebut lesPieces;;
...
plateau;;
arbre;;
resoudre (List.tl lesPieces) 1 0 1 arbre ;;
dessinerPlateau();;
let _=read_line();;
plateau;;
